<?php
    ...
    if($database_flag === false){
        $sql = "create database board_test";
        $value = mysqli_query($con, $sql) or die("Error".mysqli_error($con));
        if($value === true){
            echo "
            <script>
            alert('board_test DB가 생성되었습니다.');
            </script>
            ";
        }
    }
    ...
    $dbcon = mysqli_select_db($con, "board_test") or die("Error".mysqli_error($con));
    if(!$dbcon){
        echo "
        <script>
        alert('board_test DB가 생성되었습니다.');
